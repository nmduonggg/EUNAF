...
def fuse_classified_patch_level(p_yfs, p_masks, im_idx, eta, visualize=False, imscore=None):
    ...
    if imscore is not None:
        ...
        per_class = torch.tensor(np.stack([p0*0.0, p1*0.00, p2*0.00, p3*0.00], axis=-1))   # PxN
...
def test(eta):
    ...
    for batch_idx, (x, yt) in tqdm.tqdm(enumerate(XYtest), total=len(XYtest)):
        ...
        num_batches = max(all_imgs.size(0) // batch_size, 1)
        for i in range(num_batches):
            if i==num_batches-1:
                img = all_imgs[i*batch_size:, ...]
                gt = all_gts[i*batch_size:, ...]
            else:
                img = all_imgs[i*batch_size: (i+1)*batch_size, ...]
                gt = all_gts[i*batch_size: (i+1)*batch_size, ...]
            ...
            for i in range(len(p_yfs)):
                combine_img_lists[i] += [
                    p_yfs[i][j,...].cpu().permute(1,2,0).numpy() for j in range(p_yfs[i].size(0))]
...
if __name__ == '__main__':
    ...
    etas, flops, psnrs = [], [], []
    # for eta in [0.0]:
    for eta in np.linspace(0.5, 3.0, 30):
        print("="*20, f"eta = {eta}", "="*20)
