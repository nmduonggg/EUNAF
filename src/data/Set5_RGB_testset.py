...
class Set5_RGB_testset(Dataset):
    def __init__(self, root, scale=2, style='RGB', rgb_range=1.0):
        super(Set5_RGB_testset, self).__init__()

        self.N_raw_image = 5 #801-900
        ...
        for i in tqdm.tqdm(range(self.N_raw_image)):
            X_im_file_name = root + 'Set5/image_SRF_' + str(scale) + '/' + ('img_%03d_SRF_%d_LR.png' % (i+1,scale))
            X_data = load_image_as_Tensor(X_im_file_name, style, rgb_range)
            self.X.append(X_data)

            Y_im_file_name = root + 'Set5/image_SRF_' + str(scale) + '/' + ('img_%03d_SRF_%d_HR.png' % (i+1,scale))
